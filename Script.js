//Guarda el elemento canvas en una variable
const canvas = document.getElementById('canv');
//define el tipo de renderizado, en este caso 2d
const ctx = canvas.getContext('2d');

//Arrays que guardan todos los objetos (Circulos y paredes)
const BALLZ = [];
const WALLZ = [];

//Variables Booleanas, si son verdaderas los circulos se mueven en esa direccion
let LEFT, UP, RIGHT, DOWN;
//La velocidad es multiplicada por (1 - friction)
let friction = 0.05;
...
class Ball {
    constructor(x, y, r, m) {
        ...
        this.acceleration = 1;
        this.player = false;
        //Introduce el objeto al array BALLZ
        BALLZ.push(this);
    }

    //Dibuja los circulos en el canvas
    drawBall() {
        ctx.beginPath();
    ...
    //Muestra la aceleracion y velocidad actual de el circulo
    display() {
...
function keyControl(b) {
    //Booleanos que se vuelven verdaderos si una tecla es presionada
    ...
    //Booleanos que se vuelven falsos si una tecla deja de ser presionada
    ...
    //Si es verdadero, la aceleracion obtiene un valor
    if (LEFT) {
...
//Este loop se encarga de correr todas las funciones 60 veces por segundo
function mainLoop(timestamp) {
    ctx.clearRect(0, 0, canvas.clientWidth, canvas.clientHeight);
    //Pasa atravez de todos los circulos en el array BALLZ
...
for (let i = 0; i < 10; i++) {
    let newBall = new Ball(randInt(100, 500), randInt(50, 400), randInt(20, 50), randInt(1, 10));
    newBall.elasticity = randInt(1, 10) / 10;
}
let Wall2 = new Wall(randInt(100, 500), randInt(100, 450), randInt(100, 500), randInt(100, 450));

//Paredes conforme al tamaño de el lienzo
